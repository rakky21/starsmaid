...
import { CREATE_APPOINTMENT, GET_BOOKED_TIMES } from '../../utils/graphql.js';
...
// Format Date → "MM/DD/YYYY" for the API
...
export default function Scheduler({ onConfirmed }) {
  ...
  // Query booked times for the selected date
  ...
  const selectDay = (day) => {
    ...
    if (d < today) return;
    setSelDate(d);
    setSelTime(null); // reset time when date changes
  };
    ...
    <div className={styles.wrap}>
      <div className={styles.main}>
        ...
        <div className={styles.steps}>
          {[
            ...
            <div key={s.n} className={styles.stepGroup}>
              <div className={`${styles.step} ${s.done || stepDone(s.n) ? styles.stepDone : ''} ${stepActive(s.n) ? styles.stepActive : ''}`}>
        ...
        <div className={styles.card}>
          ...
          {!selDate && (
            <p className={styles.hint}>Please select a date first to see available times.</p>
          )}
          {selDate && (
      ...
      <aside className={styles.sidebar}>
        ...
        {selDate && selTime && selService ? (
          <div>
            <p className={styles.sumNote}>You'll receive a confirmation email and SMS after booking.</p>
...
function SumRow({ icon, label, value }) {
  return (
    <div style={{ display:'flex', alignItems:'flex-start', gap:12 }}>
      <div style={{ width:34, height:34, borderRadius:8, background:'var(--accent-light)', display:'grid', placeItems:'center', flexShrink:0, color:'var(--accent)' }}>
        {icon}
      </div>
      <div>
        <div style={{ fontSize:'.75rem', color:'var(--ink-3)', fontWeight:500, marginBottom:2 }}>{label}</div>
        <div style={{ fontSize:'.875rem', fontWeight:600, color:'var(--ink)' }}>{value}</div>
      </div>
    </div>
  );
}
